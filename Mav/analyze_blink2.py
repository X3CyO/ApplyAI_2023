...
def calculate_motion_magnitude(flow):
    # Extract the x and y components of motion vectors
    dx = flow[:, :, 0]
    dy = flow[:, :, 1]
    ...
    average_magnitude = np.mean(motion_magnitude)
...
with map_face_mesh.FaceMesh(min_detection_confidence = min_detection_confidence, min_tracking_confidence = min_tracking_confidence) as face_mesh:
    ...
    while True:
        ...
        motion_threshold = 5.0  ############# Adjust this threshold as needed

        while True:
            frame_gray = cv.cvtColor(frame, cv.COLOR_BGR2GRAY)

            # Calculate optical flow using Lucas-Kanade
            flow = cv.calcOpticalFlowPyrLK(prev_gray, frame_gray, None, None, **lk_params)

            # Calculate the magnitude of motion vectors
            motion_magnitude = calculate_motion_magnitude(flow)

            if motion_magnitude < motion_threshold:
                # Proceed with blink detection or camera analytics
